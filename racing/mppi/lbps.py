...
def _score_finite(c: np.ndarray, alpha: float, delta: float, rho: float, reward_norm: float):
    """LBPS objective for an already-filtered finite cost vector.

    Temperature search evaluates this function dozens of times per controller
    update; filtering and slicing the same cost vector on every evaluation was
    avoidable Python/NumPy overhead.
    """
    if c.size == 0:
...
def optimize_lbps_temperature(costs, *, delta: float, fallback_temperature: float, iterations: int = 32) -> LBPSResult:
    ...
    if finite.size <= 1 or spread <= 1e-12 or reward_norm <= 1e-15:
        score, ess, er = _score_finite(finite, fallback_alpha, delta, rho, reward_norm)
        return LBPSResult(float(fallback_temperature), fallback_alpha, ess, score, int(finite.size), reward_norm, er)

    score0, _, _ = _score_finite(finite, 0.0, delta, rho, reward_norm)
    alpha1 = 1.0 / spread
    score1, _, _ = _score_finite(finite, alpha1, delta, rho, reward_norm)
    ...
    if score1 > score0:
        ...
        for _ in range(40):
            nxt = prev * 2.0
            nxt_score, _, _ = _score_finite(finite, nxt, delta, rho, reward_norm)
        ...
        if not bracketed:
            score, ess, er = _score_finite(finite, prev, delta, rho, reward_norm)
    ...
    c = b - golden * (b - a)
    d = a + golden * (b - a)
    fc, _, _ = _score_finite(finite, c, delta, rho, reward_norm)
    fd, _, _ = _score_finite(finite, d, delta, rho, reward_norm)
    for _ in range(max(8, int(iterations))):
        if fc > fd:
            ...
            fd, _, _ = _score_finite(finite, d, delta, rho, reward_norm)
    alpha = 0.5 * (a + b)
    score, ess, er = _score_finite(finite, alpha, delta, rho, reward_norm)
    if score0 >= score:
        alpha = 0.0
        score, ess, er = _score_finite(finite, alpha, delta, rho, reward_norm)
