...
On ``ant-2`` they are emitted separately into antagonistic MuJoCo muscle
...
class SpikeMotorPoolSampler:
    ...
    def __init__(
        ...
        *,
        ...
        if not NUMBA_AVAILABLE:
            raise RuntimeError("spike requires Numba")
        self.n = int(n)
        self.h = int(h)
    ...
    def sample(self, rng):
        stats = _spike_motor_pool_kernel(
            rng,
            self.out,
            self.dt,
            self.thresholds_on,
        ...
        self.last_stats = {
            ...
            "spike_agonist_events_mean": float(agonist_events) / max(self.n, 1),
            "spike_antagonist_events_mean": float(antagonist_events) / max(self.n, 1),
            "spike_mean_active_units": float(mean_active_units),
            "spike_coactivation_fraction": float(coactivation_fraction),
            "spike_mean_common_drive": float(mean_drive),
            "spike_recruit_transitions_mean": float(recruit_transitions) / max(self.n, 1),
            "spike_derecruit_transitions_mean": float(derecruit_transitions) / max(self.n, 1),
            "spike_min_observed_isi_s": float(min_isi_s),
            "spike_min_rate_hz": float(self.base_rate_hz),
            "spike_max_rate_hz": float(self.max_rate_hz),
            "spike_refractory_s": float(self.refractory_s),
            "spike_drive_sigma": float(self.drive_sigma),
            "spike_drive_tau_s": float(self.drive_tau_s),
            "spike_cocontraction_drive": float(self.cocontraction_drive),
            "spike_separate_antagonists": float(self.separate_antagonists),
        }
    ...
    def estimate_unit_rms(self, *, seed: int = 918273, n_calibration: int = 256) -> float:
        ...
        if self.separate_antagonists:
            # Runtime Ant-2 proposals are centered across the rollout population.
            # Match RMS per physical muscle channel so --joint-noise has the same
            # 16-D actuator-space meaning for standard, spike and spike.
